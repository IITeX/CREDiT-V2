...
3. Deploy Internet Identity locally:
\`\`\`bash
...
\`\`\`
NEXT_PUBLIC_INTERNET_IDENTITY_CANISTER_ID=rdmx6-jaaaa-aaaah-qcaiq-cai
...
For mainnet deployment, update environment variables:
\`\`\`
...
To create your own canisters for credential storage:
...
\`\`\`json
{
  ...
  "canisters": {
    "dresume_backend": {
      "type": "motoko",
      "main": "src/dresume_backend/main.mo"
    }
  }
}
\`\`\`
...
The authentication will work with:
- Local Internet Identity (development)
- Production Internet Identity (mainnet)
- Automatic fallback and error handling
\`\`\`
</QuickEdit>

```typescriptreact file="app/profile/demo/page.tsx"
...
import { Shield, ExternalLink, Calendar, CheckCircle, Award, Briefcase, GraduationCap, Code, Trophy, MapPin, Mail, Globe, Github, Linkedin } from 'lucide-react'
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <footer className="flex flex-col gap-2 sm:flex-row py-6 w-full shrink-0 items-center px-4 md:px-6 border-t">
        <p className="text-xs text-muted-foreground">© 2025 CREDiT. Powered by Internet Computer Protocol.</p>
